...
bool SteamServer::serverInit(const String& ip, uint16 game_port, uint16 query_port, ServerMode server_mode, const String& version_number){
	...
	if(!SteamGameServer_Init(ip_address, game_port, query_port, mode, version_number.utf8().get_data())){
		return false;
	}
	return true;
}
...
bool SteamServer::sendP2PPacket(uint64_t steam_id_remote, PoolByteArray data, P2PSend send_type, int channel){
	if (SteamNetworking() == NULL) {
		return false;
	}
	CSteamID steam_id = createSteamID(steam_id_remote);
